...
const calculatePayrollDeductions = ({
  ...
  if (payrollSettings?.pf?.enabled) {
    pfAmount = Math.round(
      employeeSalary * (payrollSettings.pf.employeePercent / 100)
    );
  }

  if (
    payrollSettings?.esi?.enabled &&
    employeeSalary <= payrollSettings.esi.salaryLimit
  ) {
    esiAmount = Math.round(
      employeeSalary * (payrollSettings.esi.employeePercent / 100)
    );
  }

  if (payrollSettings?.professionalTax?.enabled) {
    const slab = payrollSettings.professionalTax.slabs?.find(
      (s) =>
        employeeSalary >= s.min &&
        employeeSalary <= s.max
    );
    if (slab) professionalTaxAmount = slab.amount;
  }

  const statutoryDeductions =
    pfAmount + esiAmount + professionalTaxAmount;

  const totalDeductions =
    attendanceDeduction + statutoryDeductions;
...
export const generatePayroll = async (req, res) => {
  try {
    ...
    const breakup = calculatePayrollDeductions({
      employeeSalary: employee.salary,
      perDaySalary,
      absentDays,
      lopLeaveDays: lopLeave,
      payrollSettings,
    });

    const netSalary = Math.max(
      employee.salary - breakup.totalDeductions,
      0
    );
    ...
    const payroll = await Payroll.create({
      ...
      pfAmount: breakup.pfAmount,
      esiAmount: breakup.esiAmount,
      professionalTaxAmount: breakup.professionalTaxAmount,
      statutoryDeductions: breakup.statutoryDeductions,
      netSalary,
  ...
  try {
    ...
    for (const emp of employees) {
      try {
        ...
        const netSalary = Math.max(
          emp.salary - breakup.totalDeductions,
          0
        );
...
export const recalculatePayroll = async (req, res) => {
  try {
    ...
    payroll.deductions = breakup.totalDeductions;
    payroll.pfAmount = breakup.pfAmount;
    payroll.esiAmount = breakup.esiAmount;
    payroll.professionalTaxAmount = breakup.professionalTaxAmount;
    payroll.statutoryDeductions = breakup.statutoryDeductions;
