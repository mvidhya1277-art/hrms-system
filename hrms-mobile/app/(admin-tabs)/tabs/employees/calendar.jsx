...
//         ? `${API_BASE_URL}/reports/attendance-leave/${targetEmployeeId}`
...
//       setDayDetails({ type: "info", label: "Future date — attendance not available" });
//       return;
//     }
...
//     if (isSunday(date)) return setDayDetails({ type: "holiday", label: "Weekly Holiday (Sunday)" });
//     if (isEvenSaturday(date)) return setDayDetails({ type: "holiday", label: "Weekly Holiday (Saturday)" });
...
//       const dateStr = `${year}-${String(currentMonth.month).padStart(2, "0")}-${String(d).padStart(2, "0")}`;

//       if (date > today || isSunday(date) || isEvenSaturday(date) || getHolidayName(dateStr)) continue;
...
//       <Text style={{ fontSize: 18, marginBottom: 6 }}>{employeeName} Attendance</Text>
...
//           {dayDetails?.type === "holiday" && <Text>Holiday: {dayDetails.label}</Text>}
//           {dayDetails?.type === "info" && <Text>{dayDetails.label}</Text>}
//           {dayDetails && !dayDetails.type && <Text>Status: {dayDetails.status}</Text>}
...
}//router.push("/employees/all-payrolls");
